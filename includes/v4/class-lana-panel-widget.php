...
class Lana_Panel_Widget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		 ...
		 * elements
		 */
		$before_panel = '<div class="card %s">';
		$after_panel  = '</div>';

		$before_panel_title = '<h3 class="card-header">';
		$after_panel_title  = '</h3>';

		$before_panel_body = '<div class="card-body">';
		$after_panel_body  = '</div>';

		$before_panel_footer = '<div class="card-footer">';
        ...
        <div class="lana-widgets-panel-widget" data-widget-id="<?php echo esc_attr( $this->id ); ?>">
            ...
            <p>
                ...
                <select name="<?php echo $this->get_field_name( 'type' ); ?>"
                    ...
                    <option value="" <?php selected( $instance['type'], '' ); ?>>
						<?php esc_html_e( 'Default', 'lana-widgets' ); ?>
                    </option>
                    <option value="text-white bg-primary" <?php selected( $instance['type'], 'text-white bg-primary' ); ?>>
						<?php esc_html_e( 'Primary', 'lana-widgets' ); ?>
                    </option>
                    <option value="text-white bg-secondary" <?php selected( $instance['type'], 'text-white bg-secondary' ); ?>>
						<?php esc_html_e( 'Secondary', 'lana-widgets' ); ?>
                    </option>
                    <option value="text-white bg-success" <?php selected( $instance['type'], 'text-white bg-success' ); ?>>
						<?php esc_html_e( 'Success', 'lana-widgets' ); ?>
                    </option>
                    <option value="text-white bg-info" <?php selected( $instance['type'], 'text-white bg-info' ); ?>>
						<?php esc_html_e( 'Info', 'lana-widgets' ); ?>
                    </option>
                    <option value="text-white bg-warning" <?php selected( $instance['type'], 'text-white bg-warning' ); ?>>
						<?php esc_html_e( 'Warning', 'lana-widgets' ); ?>
                    </option>
                    <option value="text-white bg-danger" <?php selected( $instance['type'], 'text-white bg-danger' ); ?>>
						<?php esc_html_e( 'Danger', 'lana-widgets' ); ?>
                    </option>
                    <option value="bg-light" <?php selected( $instance['type'], 'bg-light' ); ?>>
						<?php esc_html_e( 'Light', 'lana-widgets' ); ?>
                    </option>
                    <option value="text-white bg-dark" <?php selected( $instance['type'], 'text-white bg-dark' ); ?>>
						<?php esc_html_e( 'Dark', 'lana-widgets' ); ?>
                    </option>
                </select>
            </p>
            <p>
